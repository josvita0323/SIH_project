...
export function HRSafetyCoordinatorDashboard({ onBack }: { onBack: () => void }) {
  const [selectedDate, setSelectedDate] = useState<Date | undefined>(new Date())
  const safetyDocs = mockDocuments.filter((d) => d.type === "safety-circular")
  ...
  return (
    <main className="min-h-screen bg-slate-50">
      <header className="bg-white border-b sticky top-0 z-40">
        <div className="px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold">KMRL Safety & HR</h1>
              <p className="text-sm text-muted-foreground">HR & Safety Coordinator</p>
            </div>
          </div>
      ...
      <section className="p-6">
        ...
        <InboxList role="hr-safety-coordinator" />
      </section>
    </main>
  )
}
